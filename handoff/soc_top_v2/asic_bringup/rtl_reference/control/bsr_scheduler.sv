// bsr_scheduler.sv — BSR sparse scheduler for 16×16 weight-stationary systolic array
...
//
// Direct SRAM-wrapper wires — no meta decoder. Scheduler drives explicit
// read-enable/address ports and captures the synchronous read data one cycle
// later. load_weight_r is delayed 1 cycle to match buffer latency.
...
module bsr_scheduler #(
    ...
    parameter BLOCK_SIZE    = 16    // Must match systolic array (16×16)
)(
    input  wire                 clk,
    input  wire                 rst_n,
    ...
    // Direct SRAM-wrapper read: row_ptr (synchronous 1-cycle latency)
    output reg                  row_ptr_rd_en,
    output reg  [BRAM_ADDR_W-1:0] row_ptr_rd_addr,
    input  wire [31:0]            row_ptr_rd_data,

    // Direct SRAM-wrapper read: col_idx (synchronous 1-cycle latency)
    ...
    localparam LOAD_CNT_MAX   = BLOCK_SIZE - 1;           // 15
    localparam BLOCK_SHIFT    = $clog2(BLOCK_SIZE);
    // Stream counter: 16 feed + 15 skew + 15 horizontal drain = 46 cycles
    // a[15] fed at cycle 15, exits row 15 skew at cycle 30, reaches col 15 at cycle 45
    localparam STREAM_CNT_MAX = 3 * BLOCK_SIZE - 3;       // 45
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state)
        ...
        // ---- Fetch row_ptr[k] ----
        // Cycle 0: assert read enable with address
