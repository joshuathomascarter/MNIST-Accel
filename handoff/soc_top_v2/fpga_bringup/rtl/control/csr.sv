...
module csr #(
  ...
  // OpenLane bring-up uses a single ungated clock. Keep the legacy latch-based
  // gate only for non-ASIC simulation/FPGA-oriented builds.
  `ifdef VERILATOR
  ...
  // Layer management registers (multi-layer pipeline)
  localparam LAYER_TOTAL     = 8'hE0; // R/W  - total layers in network [7:0]
  localparam LAYER_CURRENT   = 8'hE4; // R/W  - current layer index [7:0]
  localparam LAYER_CONFIG    = 8'hE8; // R/W  - [0]=pool_en, [31:16]=out_h, [15:8]=reserved
  localparam LAYER_OUT_DIM   = 8'hEC; // R/W  - [31:16]=output_h, [15:0]=output_w
  localparam LAYER_STATUS    = 8'hF0; // RO   - [0]=layer_done, [1]=last_layer_done, [2]=feedback_busy
  ...
  always @(posedge clk_gated or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (layer_done_in)          st_layer_done <= 1'b1;
      if (last_layer_done_in)     st_last_layer_done <= 1'b1;
      ...
      if (csr_wen) begin
        case (csr_addr)
          ...
          LAYER_STATUS: begin
            // W1C for layer status bits
            if (csr_wdata[0]) st_layer_done      <= 1'b0;
            if (csr_wdata[1]) st_last_layer_done <= 1'b0;
          end
          BSR_STATUS: begin
  ...
  always @(*) begin
    ...
    if (csr_ren) begin
      unique case (csr_addr)
        ...
        // Layer management readback
        LAYER_TOTAL:   csr_rdata = {24'b0, r_layer_total};
        LAYER_CURRENT: csr_rdata = {24'b0, r_layer_current};
        LAYER_CONFIG:  csr_rdata = {31'b0, r_pool_en};
        LAYER_OUT_DIM: csr_rdata = {r_output_h, r_output_w};
        LAYER_STATUS:  csr_rdata = {29'b0, 1'b0, st_last_layer_done, st_layer_done};
