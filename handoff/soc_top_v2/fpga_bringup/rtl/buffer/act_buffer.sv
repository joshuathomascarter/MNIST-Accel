...
module act_buffer #(
    ...
    always @(posedge clk) begin
        ...
        if (we && rd_en) assert (bank_sel_wr != bank_sel_rd)
            else $error("act_buffer: single-port bank collision on bank %0d", bank_sel_wr);
    end
    ...
    assign a_vec = rd_en ? ((bank_sel_rd == 1'b0) ? bank0_rdata : bank1_rdata)
                         : {TM*8{1'b0}};
