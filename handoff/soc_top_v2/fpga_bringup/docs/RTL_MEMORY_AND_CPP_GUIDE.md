...
### What You Have (16×16 Systolic Array on Pynq-Z2)

| Component | Current Implementation | Memory Type |
|-----------|----------------------|-------------|
| Activation Buffer | Inline BRAM (1024×128b) | ~16 KB |
| Weight Block BRAM | Inline BRAM (1024×128b) | ~16 KB |
| Row Pointer BRAM | Inline BRAM (1024×32b) | ~4 KB |
| Column Index BRAM | Inline BRAM (1024×16b) | ~2 KB |
| Output Accumulator | Double-buffered (2×256×32b) | ~2 KB |
| **Total On-Chip** | | **~40 KB** |

### Current Data Flow

```
DDR → AXI HP (64-bit) → act_dma/bsr_dma → dma_pack_128 → BRAM → Systolic → Output Accum → DDR
                        (2:1 arbiter)      (64→128 packer)
```

### Memory-Bound Analysis

**Compute Capacity:**
- 256 PEs × 1 MAC/cycle × 100 MHz = **25.6 GOPS**

**Bandwidth Required (to sustain full utilization):**
- 256 MACs × 2 bytes (weight + activation) × 100 MHz = **51.2 GB/s**
...
| **Weight-Stationary Dataflow** | Weights loaded once, activations stream | 16× weight reuse per block |
| **64→128 Packer** | Converts AXI beats to BRAM width | No bandwidth waste on width mismatch |
...
void run_benchmark() {
    ...
    // Roofline
    printf("\n=== Roofline Model ===\n");
    printf("Peak Compute: 25.6 GOPS\n");
    printf("Peak Bandwidth: 4.0 GB/s\n");
    printf("Ridge Point: 6.4 ops/byte\n");
    printf("Your AI: %.2f (%.1f%% of ridge)\n", 
           metrics.arithmetic_intensity,
           metrics.arithmetic_intensity / 6.4 * 100);
}
```
...
> memory-bound - my 256-PE array can do 25.6 GOPS but the HP port only delivers 
> ~4 GB/s. I addressed this through:
> 
> 1. **BSR compression** - skipping zero blocks reduces effective bandwidth demand
> 2. **Weight-stationary dataflow** - each weight is reused 16× per block
> 3. **Double-buffered output accumulator** - overlaps compute with DMA
> 4. **64→128 bit packing** - eliminates width mismatch waste
>
...
void test_dense_to_bsr() {
    // 32x32 dense matrix (4 blocks of 16x16)
    std::vector<int8_t> dense(32 * 32, 1);
    ...
    BSREncoder encoder;
    auto bsr = encoder.encode(dense.data(), 32, 32);
