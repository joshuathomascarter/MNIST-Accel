...
- **16×16 weight-stationary systolic array** (256 INT8 MACs per cycle)
...
Block Sparse Row encodes only non-zero 16×16 blocks:
...
| `data[nnz_blocks × 256]` | Flattened 16×16 INT8 weight blocks |
...
256 PEs arranged in a 16-row × 16-column grid. Weight-stationary dataflow:

1. **Load phase** (16 cycles) — Weights loaded systolically, one row per cycle
2. **Compute phase** (K + 15 cycles) — Activations stream with row skew
3. **Drain phase** — Partial sums read from right edge of each row

Parameters:
- `N_ROWS = 16`, `N_COLS = 16` (256 DSPs — exceeds Z7020's 220 DSP48E1 budget)
- `BLOCK_SIZE = 16` (BSR block dimensions match array size)
...
|--------|------|-------|---------|-------|
| Activation buffer | 1 KB | 128-bit (16 × INT8) | 2 cycles | Double-buffered (ping-pong) |
| Weight buffer | 1 KB | 128-bit (16 × INT8) | 2 cycles | Single-port BRAM |
| Output accumulator | 256 × 32-bit | 32-bit | 1 cycle | Per-PE INT32 accumulation |
...
- **LOAD_WGT**: Load 16×16 weight block into systolic array (16 cycles)
- **WAIT_WGT**: Allow weight propagation through systolic chain
- **STREAM_ACT**: Stream activation tile through array (16 + 15 cycles)
...
├── systolic_array_sparse.sv   (16×16 PE array)
│   └── pe.sv × 256            (processing elements)
│       └── mac8.sv × 256      (INT8 MAC units)
