...
| `tb_soc_top.sv` | 9 | Baseline full-SoC regression for `soc_top_v2` |
| `tb_soc_top_inr.sv` | 11 | Novelty-on full-SoC INR regression with live mesh injection and root-side collapse checks |
| `tb_soc_top_inr_metadata.sv` | 12 | Full-SoC metadata-on INR regression that programs router-local targets through the accelerator CSR window |
| `tb_innet_reduce_e2e.sv` | Focused | Corrected multiroot RTL reduction benchmark for corner, edge, and interior roots |
| `tb_innet_reduce_metadata.sv` | Focused | Explicit subtree-metadata validation for a sparse-subset reduction tree that geometry-only inference cannot represent |
...
### SoC Regressions
```bash
# Baseline full-SoC regression
verilator --binary --sv --timing -f hw/sim/sv/filelist.f \
    hw/sim/sv/tb_soc_top.sv --top-module tb_soc_top --Mdir hw/sim/sv/obj_dir_soc_top
./hw/sim/sv/obj_dir_soc_top/Vtb_soc_top

# Novelty-on full-SoC INR regression
verilator --binary --sv --timing -f hw/sim/sv/filelist.f \
    hw/sim/sv/tb_soc_top_inr.sv --top-module tb_soc_top_inr --Mdir hw/sim/sv/obj_dir_soc_top_inr
./hw/sim/sv/obj_dir_soc_top_inr/Vtb_soc_top_inr

# Full-SoC metadata-programmed INR regression
verilator --binary --sv --timing -f hw/sim/sv/filelist.f \
    hw/sim/sv/tb_soc_top_inr_metadata.sv --top-module tb_soc_top_inr_metadata \
    --Mdir hw/sim/sv/obj_dir_soc_top_inr_metadata
./hw/sim/sv/obj_dir_soc_top_inr_metadata/Vtb_soc_top_inr_metadata

# Focused multiroot reduction benchmark
verilator --binary --sv --timing -f hw/sim/sv/filelist.f \
    hw/sim/sv/tb_innet_reduce_e2e.sv --top-module tb_innet_reduce_e2e \
    -GINNET_REDUCE=1 -GROOT_NODE=5 --Mdir hw/sim/sv/obj_dir_tb_innet_reduce_e2e
./hw/sim/sv/obj_dir_tb_innet_reduce_e2e/Vtb_innet_reduce_e2e

# Explicit subtree-metadata reduction benchmark
verilator --binary --sv --timing -f hw/sim/sv/filelist.f \
    hw/sim/sv/tb_innet_reduce_metadata.sv --top-module tb_innet_reduce_metadata \
    -GINNET_REDUCE=1 --Mdir hw/sim/sv/obj_dir_tb_innet_reduce_metadata
./hw/sim/sv/obj_dir_tb_innet_reduce_metadata/Vtb_innet_reduce_metadata
```
