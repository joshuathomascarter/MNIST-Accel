...
def create_sparse_mask(shape: Tuple[int, int], sparsity_pct: float, block_size: int = 14, seed: int = 42) -> np.ndarray:
    """
...
def create_transformer_qkv_weights(
    ...
    Q_mask = create_sparse_mask(Q.shape, sparsity_pct, block_size=14, seed=seed)
    K_mask = create_sparse_mask(K.shape, sparsity_pct, block_size=14, seed=seed + 1)
    V_mask = create_sparse_mask(V.shape, sparsity_pct, block_size=14, seed=seed + 2)
...
def export_transformer_fixtures(output_dir: str = None):
    ...
    for sparsity in [80.0, 90.0]:
        ...
        for name, weight in [("Q", qkv["Q"]), ("K", qkv["K"]), ("V", qkv["V"])]:
            bsr = build_bsr_from_dense(weight, 14, 14)
