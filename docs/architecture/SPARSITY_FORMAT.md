...
- **Standard block size**: 16×16 elements (configurable)
...
- **Block size**: 256 bytes for 16×16 blocks
...
2. **For each block in row:**
   ...
   - Read 256-byte block from `data` array at offset `block_idx * 64`
