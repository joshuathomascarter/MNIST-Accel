...
**16×16 Sparse Systolic Array Accelerator for PYNQ-Z2**
...
ACCEL-v1 is a hardware accelerator for sparse matrix multiplication using Block Sparse Row (BSR) format. It implements a **16×16 systolic array** with row-stationary dataflow, optimized for CNN inference workloads.

**Key Features:**
- **16×16 systolic array (256 PEs)** — scaled up from original 8×8 design
- INT8 quantization with saturation
- BSR sparse format support (70-90% sparsity)
- AXI4 interfaces for PS-PL communication
- 25.6 GOPS dense @ 100 MHz, up to 256 effective GOPS sparse

**Recent Changes (v2.0):**
- Scaled systolic array from 8×8 to 16×16 (4× throughput)
- Added parameterized `BLOCK_SIZE` throughout scheduler
- Added output accumulator with double-buffering
- Fixed BSR DMA multi-block transfer bug
- Added Python PYNQ drivers
