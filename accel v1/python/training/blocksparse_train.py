...
def compute_block_norms(weight_tensor: torch.Tensor, block_h: int, block_w: int) -> Tuple[torch.Tensor, Tuple[int, int], Tuple[int, ...]]:
    """
...
def progressive_sparsity(model: nn.Module, device: torch.device, masks: Dict[str, torch.Tensor], target_sparsity: float = 0.9):
    """
    ...
    for phase, sparsity in enumerate(sparsity_schedule):
        ...
        train_with_group_lasso(model, device, masks, epochs=fine_tune_epochs, lr=fine_tune_lr, current_sparsity=sparsity)
...
def train_with_group_lasso(model: nn.Module, device: torch.device, masks: Dict[str, torch.Tensor], epochs: int = 3, lr: float = 1e-4, current_sparsity: float = 0.0):
    """
