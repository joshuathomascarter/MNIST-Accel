...
def save_bsr_metadata(bsr_data: Dict, filepath: str, layer_name: str = ""):
    ...
    metadata = {
        ...
        "avg_tiles_per_row": float(bsr_data["num_blocks"] / bsr_data["num_block_rows"]) 
                             if bsr_data["num_block_rows"] > 0 else 0.0
    }
...
# ----------------------------
def export_layer_bsr(name: str, weight: np.ndarray, block_h: int, block_w: int, 
                     output_dir: str, quantize: bool = False, scales: Optional[np.ndarray] = None):
    """
    ...
    print(f"  Non-zero blocks: {bsr_data['num_blocks']}/{bsr_data['num_block_rows']*bsr_data['num_block_cols']} "
...
def export_model_bsr(model: nn.Module, output_dir: str, quantize: bool = False):
    ...
    summary = {
        ...
        "overall_block_sparsity_pct": float((1.0 - total_nonzero_blocks / total_blocks) * 100) if total_blocks > 0 else 0.0,
        "layers": layer_stats
    }
        ...
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            if key_data in sparse_data:
                ...
                bsr_matrix = sp.bsr_matrix(
                    (bsr_blocks, bsr_indices, bsr_indptr),
...
if __name__ == "__main__":
    ...
    summary = export_model_bsr(
        model=model,
        output_dir=OUTPUT_DIR,
        quantize=False  # Set to True when quantization is integrated
    )
