...
class AXIMasterSim:
    ...
    def get_metrics(self) -> Dict:
        ...
        avg_write_latency = (
            self.total_write_latency_ns / self.write_txn_count
            if self.write_txn_count > 0
            else 0
        )
        avg_read_latency = (
            self.total_read_latency_ns / self.read_txn_count
            if self.read_txn_count > 0
            else 0
        )
    ...
    def print_metrics(self):
        ...
        print(f"\n[{self.name}] Metrics:")
        print(f"  Transactions: {metrics['total_transactions']} (WR={metrics['write_transactions']}, RD={metrics['read_transactions']})")
