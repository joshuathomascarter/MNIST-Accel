...
def test_weight_scale_calculation():
    ...
    assert np.isclose(scale, expected_scale, rtol=1e-5), \
        f"Scale mismatch: {scale} vs {expected_scale}"
    
    # Check quantized values are clipped to [-128, 127]
    assert np.all(weights_int8 >= -128) and np.all(weights_int8 <= 127), \
        "Quantized values out of INT8 range"
    
    # Check largest value quantizes to ±127
    max_val_idx = np.argmax(np.abs(weights))
    assert np.abs(weights_int8.flat[max_val_idx]) == 127, \
