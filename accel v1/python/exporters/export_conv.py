...
def create_conv_weights(in_channels: int = 32, out_channels: int = 64,
                        kernel_size: int = 3, sparsity_pct: float = 70.0,
                        seed: int = 42) -> dict:
...
def export_conv_fixtures(output_dir: str = None):
    ...
    for cfg in configs:
        ...
        save_bsr_metadata(bsr, os.path.join(bsr_dir, 'weights.meta.json'), layer_name=f"conv_{cfg['in_ch']}_{cfg['out_ch']}")
