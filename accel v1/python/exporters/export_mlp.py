...
def create_mlp_weights(input_dim: int = 512, output_dim: int = 128,
                       sparsity_pct: float = 90.0, seed: int = 42) -> dict:
...
def export_mlp_fixtures(output_dir: str = None):
    ...
    for cfg in configs:
        ...
        save_bsr_metadata(bsr, os.path.join(bsr_dir, 'weights.meta.json'), layer_name=f"fc_{cfg['input_dim']}_{cfg['output_dim']}")
