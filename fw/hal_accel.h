...
#define OP_REDUCE    0x06u   /* Emit N_ROWS MSG_REDUCE flits to root */
...
/* Issue OP_REDUCE: emit N_ROWS MSG_REDUCE flits to root tile.
 *   sp_result_base : scratchpad word offset of the N_ROWS INT32 results
 *   reduce_id_base : reduce ID for row 0; row r uses (reduce_id_base + r) & 0xFF
 *   reduce_expect  : number of contributing tiles (0..15)
 *   root_tile      : destination tile (NoC node ID)
 * The tile controller emits one FLIT_HEADTAIL MSG_REDUCE per output row.
 */
static inline void accel_reduce(uint32_t tile,
                                uint32_t sp_result_base,
                                uint8_t  reduce_id_base,
                                uint8_t  reduce_expect,
                                uint8_t  root_tile)
{
    accel_wait_idle(tile);
    accel_cmd(tile, OP_REDUCE,
              sp_result_base,
              ((uint32_t)reduce_expect << 12) |
              ((uint32_t)reduce_id_base <<  4) |
              ((uint32_t)root_tile      &  0xFu),
              0u);
}
