// tiling.cpp — Tile size calculation for 16×16 systolic array mapping
// =============================================================================
//
// Implements the tiling strategy that maps arbitrary GEMM operations onto a
// 16×16 weight-stationary systolic array.  Produces TileDescriptor arrays
// consumed by BufferManager::executeTileSequence().
//
// Key invariants:
//   - All dimensions are padded UP to multiples of 16 (zero-pad)
//   - Weight tiles are 16×16 INT8 → 256 bytes each
//   - Activation tiles are 16×16 INT8 → 256 bytes each
//   - Output tiles are 16×16 INT32 → 1024 bytes each
                         ...
                         uint32_t out_base)
{
    ...
    plan.wgt_tile_bytes = TILE_ELEMS;                          // 256 bytes INT8
    plan.act_tile_bytes = TILE_ELEMS;                          // 256 bytes INT8
    plan.out_tile_bytes = TILE_ELEMS * sizeof(int32_t);        // 1024 bytes INT32
                          ...
                          uint32_t out_base)
{
    ...
    // Sanity: BSR block rows should match padded M / 16
    ...
    //   block i starts at wgt_base + i * 256.
    //   (This matches bsr_dma.sv which reads blocks at base + blk_idx*256.)
    //
    // Activation layout:  standard tiled layout (same as dense).
    //   tile(k, n) at act_base + (k * num_n_tiles + n) * 256.
