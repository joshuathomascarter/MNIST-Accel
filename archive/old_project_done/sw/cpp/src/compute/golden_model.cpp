...
// Tiled GEMM replicates the exact 16×16 tiling the hardware executes.
//
// =============================================================================
...
// =============================================================================
// Tiled GEMM:  Same result, computed in 16×16 blocks
// =============================================================================

std::vector<int32_t> GoldenModel::tiledGemmINT8(const int8_t* A, const int8_t* B,
                                                 uint32_t M, uint32_t N, uint32_t K) {
    const uint32_t T = TILE_DIM;
    uint32_t Mp = padTo16(M);
    uint32_t Np = padTo16(N);
    uint32_t Kp = padTo16(K);
    ...
    for (uint32_t mt = 0; mt < nM; ++mt) {
        for (uint32_t nt = 0; nt < nN; ++nt) {
            ...
            for (uint32_t kt = 0; kt < nK; ++kt) {
                // Compute 16×16 × 16×16 partial product and accumulate
