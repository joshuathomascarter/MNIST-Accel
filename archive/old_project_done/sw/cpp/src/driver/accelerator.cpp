...
// single coherent inference driver for the 16×16 INT8 systolic array on
// PYNQ-Z2.
//
// =============================================================================
...
void Accelerator::loadMNISTWeights(const std::string& base_dir) {
    ...
    // Configure pooling and output dimensions for MNIST layers
    // PyTorch model: conv1(3×3)→ReLU→conv2(3×3)→ReLU→pool2d(2)→flatten→fc1→fc2
    // Only conv2 has pooling; fc1 input = 64*12*12 = 9216 confirms this.
    if (auto* l = findLayer("conv1")) {
        l->pool_after = false;  // No pooling after conv1
        l->output_h = 26;      // 28-3+1 = 26
        l->output_w = 26;
    }
    if (auto* l = findLayer("conv2")) {
        l->pool_after = true;   // 2×2 max pool after conv2 only
        l->output_h = 24;       // 26-3+1 = 24 (pre-pool), 12 after pool
        l->output_w = 24;
    }
    if (auto* l = findLayer("fc1")) {
        l->pool_after = false;
        l->output_h = 1;
        l->output_w = 140;
    }
    if (auto* l = findLayer("fc2")) {
        l->pool_after = false;
        l->output_h = 1;
        l->output_w = 10;
    }
...
InferenceResult Accelerator::runMNIST(const int8_t* input_28x28) {
    ...
    // Configure multi-layer pipeline: total layers and initial layer index
    const uint8_t total_layers = static_cast<uint8_t>(plans.size());
    csr_->setLayerTotal(total_layers);
    ...
    for (size_t i = 0; i < plans.size(); ++i) {
        ...
        // Set current layer index (hardware uses this to decide feedback vs DDR)
        csr_->setLayerCurrent(static_cast<uint8_t>(i));

        // Configure pooling for this layer
        csr_->setPoolEnable(layer->pool_after);
        csr_->setOutputDimensions(layer->output_h, layer->output_w);

        // Configure CSR matrix/tile dimensions
        csr_->setMatrixDimensions(plan.padded.M_padded,
                                  plan.padded.N_padded,
                                  plan.padded.K_padded);
        csr_->setTileDimensions(plan.grid.num_m_tiles,
        ...
        // For subsequent layers, the hardware feedback path has already
        // loaded the previous layer's output into the act_buffer.
        if (i == 0) {
        ...
        // On intermediate layers, hw feeds output back to act_buffer (layer_done)
        // On last layer, hw drains to DDR (last_layer_done)
        if (i < plans.size() - 1) {
            // Intermediate layer: wait for feedback to complete
            csr_->waitLayerDone();
            csr_->clearLayerDone();
        } else {
            // Last layer: wait for DDR write to complete
            csr_->waitDone();
        }

        if (config_.enable_logging) {
            std::cout << "[ACCEL] Layer '" << ln << "' complete"
                      << (layer->pool_after ? " (pooled)" : "") << "\n";
        }
    }
