...
void CSRInterface::setLayerTotal(uint8_t total) {
    write32(REG_LAYER_TOTAL, static_cast<uint32_t>(total));
}

void CSRInterface::setLayerCurrent(uint8_t layer) {
    write32(REG_LAYER_CURRENT, static_cast<uint32_t>(layer));
}

void CSRInterface::setPoolEnable(bool enable) {
    write32(REG_LAYER_CONFIG, enable ? layer_config::POOL_EN_BIT : 0);
}

void CSRInterface::setOutputDimensions(uint16_t height, uint16_t width) {
    uint32_t val = (static_cast<uint32_t>(height) << 16) | static_cast<uint32_t>(width);
    write32(REG_LAYER_OUT_DIM, val);
}

uint8_t CSRInterface::getLayerCurrent() const {
    return static_cast<uint8_t>(read32(REG_LAYER_CURRENT) & 0xFF);
}

bool CSRInterface::isLayerDone() const {
    return (read32(REG_LAYER_STATUS) & layer_status::LAYER_DONE_BIT) != 0;
}

bool CSRInterface::isLastLayerDone() const {
    return (read32(REG_LAYER_STATUS) & layer_status::LAST_LAYER_DONE_BIT) != 0;
}

void CSRInterface::clearLayerDone() {
    write32(REG_LAYER_STATUS, layer_status::LAYER_DONE_BIT);  // W1C
}

void CSRInterface::clearLastLayerDone() {
    write32(REG_LAYER_STATUS, layer_status::LAST_LAYER_DONE_BIT);  // W1C
}

bool CSRInterface::waitLayerDone(uint32_t timeout_us) const {
    return pollBits(REG_LAYER_STATUS,
                    layer_status::LAYER_DONE_BIT | layer_status::LAST_LAYER_DONE_BIT,
                    0,  // We want ANY bit set, so we poll != 0
                    timeout_us);
}
