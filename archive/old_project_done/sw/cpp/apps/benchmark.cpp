...
struct BenchOptions {
    ...
    std::string bsr_dir     = "data/bsr_export_16x16";
...
static void runLayerProfile(const BenchOptions& opts) {
    ...
    for (size_t i = 0; i < plans.size(); ++i) {
        ...
        snap.total_cycles   = p.totalTiles() * 256;  // 256 cycles per tile (16×16)
...
static void runBatchComparison(const BenchOptions& opts) {
    ...
    // Roofline constants
    constexpr double PEAK_GOPS = 25.6;   // 256 PEs × 100 MHz
    constexpr double PEAK_BW   = 1200.0; // 32-bit AXI HP @ 150 MHz (MB/s)
    const double ridge_point   = PEAK_GOPS / (PEAK_BW / 1000.0);

    std::fprintf(stdout, "Architecture: 16×16 weight-stationary systolic array\n");
    ...
    std::fprintf(stdout, "Batch size:    N=%u (%u N-tiles → weight reuse in M→K→N scheduling)\n",
                 B, (B + 15) / 16);
    std::fprintf(stdout, "FC2 caching:   ON (all 2,560 B weight tiles fit in BRAM)\n\n");
    ...
    std::fprintf(stdout, "     On-chip BRAM accumulators hold partial output sums (%.1f KB).\n",
                 (double)((batched[2].grid.num_m_tiles * batched[2].grid.num_n_tiles * 1024)) / 1024.0);
    std::fprintf(stdout, "FC2: AI=%.2f still below ridge (%.2f).\n",
                 batched[3].arithmeticIntensity(), ridge_point);
    std::fprintf(stdout, "     FC2 is only %u tiles × 256 cycles = %u cycles (%.1f µs @ 100 MHz).\n",
                 batched[3].totalTiles(),
                 batched[3].totalTiles() * 256,
                 batched[3].totalTiles() * 256 / 100.0);
