...
// =============================================================================
//
// Maps arbitrary M×K × K×N GEMM operations onto a 16×16 weight-stationary
...
//   ▸ Outer M-N: each (m,n) pair produces one 16×16 output block in C.
...
// Hardware constants
// =============================================================================
constexpr uint32_t SYSTOLIC_ROWS = 16;
constexpr uint32_t SYSTOLIC_COLS = 16;
constexpr uint32_t TILE_DIM      = 16;
constexpr uint32_t TILE_ELEMS    = TILE_DIM * TILE_DIM;  // 256
...
// =============================================================================
struct PaddedShape {
    uint32_t M_padded;      // ceil(M / 16) * 16
    uint32_t N_padded;      // ceil(N / 16) * 16
    uint32_t K_padded;      // ceil(K / 16) * 16
};
...
// =============================================================================
struct TileGrid {
    uint32_t num_m_tiles;   // ceil(M_padded / 16)
    uint32_t num_n_tiles;   // ceil(N_padded / 16)
    uint32_t num_k_tiles;   // ceil(K_padded / 16)
...
struct TilingPlan {
    ...
    uint32_t wgt_tile_bytes;        // 16*16*sizeof(int8) = 256
    uint32_t act_tile_bytes;        // 16*16*sizeof(int8) = 256
    uint32_t out_tile_bytes;        // 16*16*sizeof(int32) = 1024
    ...
    uint32_t nnz_weight_blocks = 0;      // Non-zero 16×16 weight blocks
...
/// Round up x to the nearest multiple of 16
constexpr uint32_t padTo16(uint32_t x) {
...
namespace mnist {
    ...
    ///
    /// @param bsr_dir     Path to BSR export (e.g. "data/bsr_export_16x16")
