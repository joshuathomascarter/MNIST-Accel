...
//   values[nnz_blocks × 256]     : flattened 16×16 INT8 blocks, row-major
//
...
constexpr uint32_t BSR_BLOCK_DIM = 16;
constexpr uint32_t BSR_BLOCK_SIZE = BSR_BLOCK_DIM * BSR_BLOCK_DIM;  // 256

// =============================================================================
// BSRMatrix — In-memory representation of a BSR-encoded weight matrix
// =============================================================================
struct BSRMatrix {
    uint32_t num_block_rows;    // M_padded / 16
    uint32_t num_block_cols;    // K_padded / 16
    uint32_t nnz_blocks;        // Number of non-zero 16×16 blocks
    ...
    std::vector<int8_t>   values;      // [nnz_blocks * 256], flattened block data
...
public:
    ...
    /// @param rows    Number of rows (will be padded to multiple of 16)
    /// @param cols    Number of cols (will be padded to multiple of 16)
...
private:
    ...
    /// Check if a 16×16 block is "zero" (all |values| <= threshold)
