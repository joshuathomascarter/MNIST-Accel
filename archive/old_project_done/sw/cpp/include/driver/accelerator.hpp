...
// =============================================================================
//
// High-level API for the 16×16 INT8 weight-stationary systolic array
...
//   Accelerator accel;
//   accel.init();
//   accel.loadWeights("data/bsr_export_16x16");
...
struct LayerWeights {
    ...
    float weight_scale;
    float bias_scale;
    bool pool_after;            // Apply 2×2 max pool after this layer
    uint16_t output_h;          // Output feature map height
    uint16_t output_w;          // Output feature map width
};
...
public:
    ...
    /// @param dir       Path to export dir (e.g. "data/bsr_export_16x16/conv1")
    ...
    /// @param base_dir  e.g. "data/bsr_export_16x16"
    ...
    /// Uses multi-layer hardware pipeline: intermediate layer outputs stay
    /// in on-chip BRAM and feed back as next layer's activations.
    /// Only the final layer's output is written to DDR.
