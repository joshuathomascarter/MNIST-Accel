...
public:
    ...
    // -------------------------------------------------------------------------
    // Layer Management (Multi-Layer Pipeline)
    // -------------------------------------------------------------------------
    
    // Set total number of layers in the network
    void setLayerTotal(uint8_t total);
    
    // Set current layer index (0-based)
    void setLayerCurrent(uint8_t layer);
    
    // Enable/disable 2×2 max pooling on output
    void setPoolEnable(bool enable);
    
    // Set output feature map dimensions (for pooling logic)
    void setOutputDimensions(uint16_t height, uint16_t width);
    
    // Read current layer index
    uint8_t getLayerCurrent() const;
    
    // Check if current layer is complete (W1C)
    bool isLayerDone() const;
    
    // Check if last layer output is written to DDR (W1C)
    bool isLastLayerDone() const;
    
    // Clear layer_done status bit
    void clearLayerDone();
    
    // Clear last_layer_done status bit
    void clearLastLayerDone();
    
    // Wait for layer completion with timeout
    bool waitLayerDone(uint32_t timeout_us = 10000000) const;
