...
module tb_innet_reduce_e2e #(
  parameter bit INNET_REDUCE = 1'b0,
  parameter int ROOT_NODE = 0
);
  ...
  localparam int EXPECTED_CONTRIB = NUM_NODES - 1;
  localparam int EXPECTED_SUM = ((NUM_NODES * (NUM_NODES - 1)) / 2) - ROOT_NODE;
  ...
  task automatic drive_group(input int group_id);
    begin
      ...
      for (int src = 0; src < NUM_NODES; src++) begin
        if (src != ROOT_NODE)
          wait (src_credits[src] > 0);
      end

      for (int src = 0; src < NUM_NODES; src++) begin
        if (src != ROOT_NODE) begin
          local_flit_in[src]  = make_reduce_flit(
            4'(src),
            4'(ROOT_NODE),
            2'(SOURCE_VC),
            8'(group_id),
  ...
  initial begin
    for (int i = 0; i < NUM_NODES; i++) begin
      ...
      inr_meta_cfg_valid[i]     = 1'b0;
      inr_meta_cfg_reduce_id[i] = '0;
      inr_meta_cfg_target[i]    = '0;
      inr_meta_cfg_enable[i]    = 1'b0;
    end
    ...
    for (int gid = 0; gid < NUM_GROUPS; gid++) begin
      ...
      if (!u_root.reduce_result_valid[gid]) begin
        ...
        $fatal(1, "tb_innet_reduce_e2e failed: group %0d incomplete", gid);
      end
    end
    ...
    for (int gid = 0; gid < NUM_GROUPS; gid++) begin
      if (u_root.reduce_results[gid] != EXPECTED_SUM) begin
        ...
        $fatal(1, "tb_innet_reduce_e2e failed: group %0d result mismatch", gid);
      end
    end

    $display("[METRIC] ROOT_NODE=%0d INNET_REDUCE=%0d root_packets=%0d link_flits=%0d cycles=%0d groups=%0d expected_sum=%0d",
             ROOT_NODE, INNET_REDUCE, root_packets_seen, link_flit_count, end_cycle - start_cycle,
             u_root.reduce_groups_completed, EXPECTED_SUM);
    $display("[PASS] tb_innet_reduce_e2e completed with correct final reductions");
    $finish;
  end
