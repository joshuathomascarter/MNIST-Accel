...
module accel_tile_matrix_map_tb;
  ...
  function automatic int expected_weight(input int row, input int col);
    begin
      expected_weight = (row * 4) + col - 32;
    end
  endfunction
  ...
  initial begin
    ...
    // Weight block: every row and column is unique.
    for (int row = 0; row < N_ROWS; row++) begin
      dut.u_sp.mem[WGT_BASE + row * 4 + 0] = pack4(expected_weight(row, 0), expected_weight(row, 1), expected_weight(row, 2), expected_weight(row, 3));
      dut.u_sp.mem[WGT_BASE + row * 4 + 1] = pack4(expected_weight(row, 4), expected_weight(row, 5), expected_weight(row, 6), expected_weight(row, 7));
      dut.u_sp.mem[WGT_BASE + row * 4 + 2] = pack4(expected_weight(row, 8), expected_weight(row, 9), expected_weight(row, 10), expected_weight(row, 11));
      dut.u_sp.mem[WGT_BASE + row * 4 + 3] = pack4(expected_weight(row, 12), expected_weight(row, 13), expected_weight(row, 14), expected_weight(row, 15));
    end
    ...
    for (int row = 0; row < N_ROWS; row++) begin
      for (int col = 0; col < N_COLS; col++) begin
        ...
        exp = expected_weight(row, col);
        if (got !== 32'(exp)) begin
          $display("row %0d col %0d got %0d expected %0d", row, col, got, exp);
          $fatal(1, "Matrix mapping mismatch at row %0d col %0d", row, col);
        end
      end
    end

    $display("accel_tile_matrix_map_tb PASS: direct tile compute preserves full row/column mapping");
    $finish;
  end

  initial begin
    #50000;
    $fatal(1, "accel_tile_matrix_map_tb timeout");
  end
