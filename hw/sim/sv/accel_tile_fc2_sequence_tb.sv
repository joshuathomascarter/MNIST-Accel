...
module accel_tile_fc2_sequence_tb;
  ...
  localparam int FC2_WEIGHT_OFFSET = 0;
  ...
  task automatic run_k_tile(input int k_tile);
    begin
      ...
      $fatal(1, "Timed out waiting for accel_tile compute to finish for k_tile=%0d", k_tile);
    end
  endtask
    ...
    begin
      ...
      for (int col = 0; col < N_COLS; col++) begin
        if (col_sums[col] !== expected[col]) begin
          ...
          $fatal(1, "FC2 k8 partial mismatch after sequence at col %0d", col);
        end
      end
    end

    $display("accel_tile_fc2_sequence_tb PASS: repeated real-data computes preserve correct final k8 partial logits");
    $finish;
  end

  initial begin
    #200000;
    $fatal(1, "accel_tile_fc2_sequence_tb timeout");
  end
