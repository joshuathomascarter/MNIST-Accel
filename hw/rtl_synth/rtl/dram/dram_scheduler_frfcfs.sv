...
module dram_scheduler_frfcfs #(
    ...
    localparam int BYTE_OFF = 1;  // 16-bit bus
    genvar gi;
    generate
    ...
    always_comb begin
        ...
        for (int i = 0; i < QUEUE_DEPTH; i++) begin
            if (is_row_hit[i] && entry_age[i] >= best_age) begin
                found_hit = 1'b1;
        ...
        if (!found_hit) begin
            best_age = '0;
            for (int i = 0; i < QUEUE_DEPTH; i++) begin
                if (is_bank_ready[i] && entry_age[i] >= best_age) begin
                    found_ready = 1'b1;
                    best_idx    = i[QIX_W-1:0];
                    best_age    = entry_age[i];
                end
    ...
    typedef enum logic [2:0] {
        SCH_IDLE,
        SCH_ISSUE_PRE,   // issue precharge for row-miss
        SCH_ISSUE_ACT,   // issue activate
        SCH_ISSUE_RW,    // issue read or write
        SCH_REFRESH       // refresh in progress
    } sch_state_t;

    sch_state_t sch_state, sch_state_next;
        ...
        case (sch_state)
            SCH_IDLE: begin
                ...
                if (ref_req && !ref_busy) begin
                    ...
                    logic [BANK_BITS-1:0] bk;
                    bk = q_bank[best_idx];
                    if (is_row_hit[best_idx]) begin
                        // Row hit → go straight to RW
                        sch_state_next = SCH_ISSUE_RW;
                    end else if (bank_row_open[bk]) begin
