...
module plic #(
  ...
  always_ff @(posedge clk) begin
    if (wvalid && wready && aw_valid) begin
      ...
      if (awaddr[31:7] == 0 && awaddr[6:2] < NUM_SOURCES) begin
        if (wstrb[0]) begin
      ...
      if (awaddr[31:6] == 32'h2000 >> 6) begin
  ...
  always_comb begin
    claim_clr = '0;
    if (wvalid && wready && aw_valid &&
        awaddr[31:2] == 32'h200004 >> 2 && wstrb[0]) begin
      for (int i = 0; i < NUM_SOURCES; i++) begin
        if (i == wdata[4:0]) claim_clr[i] = 1'b1;
      end
    end
  end
