...
module soc_top #(
  ...
  accel_top #(
    ...
    .m_axi_arlen   (/* unused — crossbar ignores burst len */),
    .m_axi_arsize  (),
    .m_axi_arburst (),
  ...
  plic #(
    ...
    .irq_i({26'b0, accel_irq, 1'b0, irq_timer_int, irq_uart, 3'b0}),
    .irq_o(irq_plic_ext),
  ...
  always_comb begin
    if (timer_rvalid) begin
      ...
      // UART already drives s_r* directly; defaults above
      s_rvalid[2] = 1'b0;
      s_rdata[2]  = '0;
      s_rresp[2]  = 2'b00;
      s_rid[2]    = '0;
  ...
  assign s_awready[2] = (periph_aw_sel == 8'h01) ? timer_awready :
                         (periph_aw_sel == 8'h02) ? gpio_awready  :
                         (periph_aw_sel == 8'h03) ? plic_awready  :
                         1'b1; // UART drives directly
  assign s_wready[2]  = (periph_aw_sel == 8'h01) ? timer_wready  :
                         (periph_aw_sel == 8'h02) ? gpio_wready   :
                         (periph_aw_sel == 8'h03) ? plic_wready   :
                         1'b1;
  assign s_arready[2] = (periph_ar_sel == 8'h01) ? timer_arready :
  ...
  // Slaves 5-7: Unused — DECERR responder
  genvar si;
  generate
    for (si = 5; si < NUM_SLAVES; si++) begin : gen_dummy_slaves
      assign s_awready[si] = 1'b1;
      assign s_wready[si]  = 1'b1;
      assign s_arready[si] = 1'b1;

      // Write response: immediate DECERR
      assign s_bvalid[si]  = 1'b0;
      assign s_bresp[si]   = 2'b11;
      assign s_bid[si]     = '0;

      // Read response: immediate DECERR
      assign s_rvalid[si]  = 1'b0;
      assign s_rdata[si]   = 32'hDEAD_DEAD;
      assign s_rresp[si]   = 2'b11;
      assign s_rid[si]     = '0;
