...
module noc_traffic_gen #(
  ...
  always_comb begin
    ...
    case (inj_state)
      INJ_HEAD: begin
        if (has_credit[inject_vc]) begin
          flit_valid = 1'b1;
          flit_out = make_head_flit(NODE_BITS'(NODE_ID),
                                    dst_node, inject_vc, MSG_DATA,
                                    {16'(cycle_count[15:0]), 32'h0});
          if (PKT_LEN == 1)
            flit_out.flit_type = FLIT_HEADTAIL;
        end
      end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // Count injected packets (on HEAD or HEADTAIL)
      ...
      if (flit_valid_in && (flit_in.flit_type == FLIT_HEAD ||
                            flit_in.flit_type == FLIT_HEADTAIL)) begin
