...
module dram_phy_simple_mem #(
    ...
    // 32-bit bus: col is already word-granular, use all bits (no DDR col[0] drop)
    function automatic logic [MEM_ADDR_W-1:0] phy_to_mem_addr(
        ...
        logic [ROW_BITS + BANK_BITS + COL_BITS - 1:0] linear_word_addr;
        begin
            linear_word_addr = {row, bank, col};
...
`ifndef SYNTHESIS
                    if ($test$plusargs("PHY_TRACE"))
                        $display("[PHY-RD] bank=%0d row=%0d col=%0d → mem[%0d] = %08h",
                                 bank, open_row[bank], dram_phy_col,
                                 phy_to_mem_addr(BANK_BITS'(bank), open_row[bank], dram_phy_col),
                                 mem[phy_to_mem_addr(BANK_BITS'(bank), open_row[bank], dram_phy_col)]);
`endif
                end
            end
        end
