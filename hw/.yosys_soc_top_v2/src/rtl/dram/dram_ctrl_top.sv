...
module dram_ctrl_top #(
    ...
    // -----------------------------------------------------------------------
    localparam int DRAM_ADDR_W = BANK_BITS + ROW_BITS + COL_BITS + 2; // +2 byte offset for 32-bit bus
        ...
        case (axi_st)
            ...
            AXI_WRITE_DATA: begin
                ...
                if (s_axi_wvalid && s_axi_wlast && wb_wr_ready)
                    axi_st_next = AXI_WRITE_WAIT_COMMIT;
            end

            AXI_WRITE_WAIT_COMMIT: begin
                // Only acknowledge the write after the queued data has been
                // drained alongside a scheduled DRAM WRITE command.
