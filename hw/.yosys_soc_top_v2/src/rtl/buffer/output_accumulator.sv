...
//   C[M×N] = A[M×K] × B[K×N]
//
// With TK=16 (our tile size), a 1024×1024 matrix requires:
//   1024 / 16 = 64 K-tiles per output tile
//
// Each K-tile produces partial sums that must be accumulated:
//   C_partial_0 = A[0:16] × B[0:16]
//   C_partial_1 = A[16:32] × B[16:32]
//   ...
...
//   - 256 accumulators (16×16 outputs)
//   - Each accumulator is 32 bits (INT32 for overflow headroom)
//   - Total per bank: 256 × 32 = 8,192 bits = 1 KB
//
...
// MAGIC NUMBERS
// =============
// 256 = 16×16 accumulators (matches systolic array output size)
...
module output_accumulator #(
    ...
    // Total accumulators = N_ROWS × N_COLS = 256.
    parameter N_COLS    = 16,
    ...
    // For 256 accumulators: need 256/8 = 32 words.
    parameter ADDR_W    = 10
)(
    // =========================================================================
    // SYSTEM INTERFACE
     ...
     * ----------------------------------------------------------------
     * Layout: [row0_col0, row0_col1, ..., row15_col15]
     * Each element is ACC_W bits (32 bits).
     * Total width: N_ROWS × N_COLS × ACC_W = 16 × 16 × 32 = 8,192 bits
     */
    ...
    // LOCAL PARAMETERS
    // =========================================================================
    localparam NUM_ACCS = N_ROWS * N_COLS;  // 256 for 16x16 array
    ...
    // =========================================================================
    /**
     * Two banks of 256 × 32-bit signed accumulators.
     * One bank accumulates while the other is read by DMA.
     * 
     * Memory usage: 2 × 256 × 32 = 16,384 bits = 2 KB
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // ─────────────────────────────────────────────────────────────────
            // All 256 accumulators update in parallel (massive parallelism).
            ...
            if (tile_done) begin
                ...
                $display("[OUT_ACCUM] tile_done: bank_sel %0b->%0b  bank0_rdy=%0b bank1_rdy=%0b  @ %0t",
                         bank_sel, ~bank_sel, 
                         (bank_sel == 1'b0) ? 1'b1 : bank0_ready,
                         (bank_sel == 1'b1) ? 1'b1 : bank1_ready,
                         $time);
